...
CONSTRAINT_PATTERNS: list[tuple[str, str]] = [
    ...
    # 用户直接报"脸部/手失真"也要变成判据：否则评估退回通用套话，判不出用户
    # 真正在意的东西（实测"脸部和手有一些失真"没匹配上，评估给了个无关的 8/10）
    (r"脸部|脸", "面部五官正常（双眼对称、无灰块/糊块）"),
    (r"失真|变形|走形|糊块|糊了|模糊|重影|崩了", "该部位无明显失真/糊块"),
    (r"手|指", "手部结构正常（无明显畸形/融合）"),
...
# 除了身体部位与区域指代词，还要覆盖口语说法（"修一下手"/"手崩了"），
# 否则"要修手"这类请求根本不会被识别为局部修复（实测漏掉过）
LOCAL_FIX_PATTERN = re.compile(
    r"修手|手部|手指|手崩|崩手|手.{0,4}修|修.{0,4}手|"
    r"指.{0,4}修|修.{0,4}指|眼.{0,4}崩|崩.{0,4}眼|"
    r"修脸|脸崩|崩脸|脸.{0,4}崩|脸.{0,4}修|修.{0,4}脸|"
    r"脸部|面部|五官|局部|扣掉|去掉|抹掉|把脸|"
