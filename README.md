...
- **改完代码必须重启服务（已强制）**：引擎每回合比对关键模块指纹，发现磁盘代码比运行中的新，就红色提示「请重启服务」并**拒绝执行生成类工具**——不会再用旧逻辑悄悄跑一遍。（这条来自一次真实事故：改了输入文件声明没重启，导致图片没上传、工具连续失败）
- **局部问题真的只改局部**：说「修手/修脸」时，引擎会自动改走 `local_repair` —— 矩形区域、用户给的黑白遮罩都走同一条「只重绘遮罩区」的链路，**区域外像素逐像素保持原图**（实测区域外 0 像素改动）。检测不到目标时如实说「没定位到要修的区域」并请你给遮罩，绝不假装修好、也不用整图重绘去凑
...
- **看图能力自带体检**：换 API 后设置面板会显示「视觉地址/Key 来源 + 自检：可用/不可用（原因）」，并在启动与保存设置时各探一次；视觉不可用时看图与图像评估会明确告知，而不是悄悄跳过。看图失败时大脑**只会如实说"我看不到这张图"**，不会编造画面、也不会拿项目旧图顶替
...
- **换了 API 之后看图/评估不工作？** 视觉地址与 Key 默认**跟随大脑**，所以只改「API 地址」即可；但视觉模型名通常和文本模型不同，必须单独填（在 ⚙ 里打开设置就能看到当前生效值与自检结论：`✅ 可用 / ❌ 不可用（原因）/ ⚠ 未确定（服务商瞬时报错）`）。若视觉要用另一家供应商，填面板里的「视觉 API 地址 / 视觉 Key」（留空即跟随大脑）。
...
comfy_agent/          执行引擎（纯标准库，可独立使用）
  ...
  mask.py             遮罩工具（PNG 覆盖率/包围盒/尺寸，纯标准库）——判定「是不是真的局部」
  freshness.py        引擎代码指纹自检（改完没重启服务 → 显式告警并拒绝跑生成，避免用旧代码白烧 GPU）
  promptspec.py       提示词家族规范（SDXL标签/LTX英文/MiniMax中文）
  templates/          7 个模板
  runner.py           执行编排
  cli.py              引擎 CLI（JSON 输出）
brain/                大脑
  llm.py              OpenAI 兼容 BYOK 客户端 + VLM（视觉地址/Key 默认跟随大脑，自带可用性探针）
